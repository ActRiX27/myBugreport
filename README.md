...
- bugreport 解析可自动识别 logcat threadtime / dmesg 结构，补充 level/tag 后驱动规则命中（示例涵盖 crash、network、SELinux）。
- 基线分析可选输出 build fingerprint 与 dropbox 段落的提示性 Finding，增强上下文可见性（不影响旧行为）。
...
> 无法接入设备时，可参考 `examples/simulated_pipeline_output.md` 查看完整流水线的模拟 CLI 日志与报告片段。
...
mybugreport-pipeline tool parse --artifacts .work/adb/collect/artifacts.json --out .work/adb/parse

# 基线分析
mybugreport-pipeline analyze .work/parse/records.jsonl .work/analyze/findings.json
mybugreport-pipeline tool analyze --records .work/adb/parse/records.jsonl --rules samples/rules --out .work/adb/analyze

# 渲染报告（Markdown + JSON）
mybugreport-pipeline report .work/analyze/findings.json .work/report/report.md --artifacts .work/collect/artifacts.json
mybugreport-pipeline tool report --artifacts .work/adb/collect/artifacts.json --findings .work/adb/analyze/findings.json --out .work/adb/report --evidence .work/adb/analyze/findings_evidence.jsonl --records .work/adb/parse/records.jsonl
...
- `mybugreport/pipeline/collect/adb.py`：ADB 采集骨架，输出 artifacts.json 与采集日志。
- `mybugreport/pipeline/parse/parsers.py`：logcat/dmesg/bugreport 解析器，输出 records.jsonl 与 parse_summary。
