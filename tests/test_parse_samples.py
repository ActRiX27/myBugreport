...
def test_bugreport_enriches_threadtime_and_dropbox():
    lines = [
        "01-01 00:00:00.000  100  200 E Tag: something happened",
        "==== dropbox entries (timestamp):",
        "data_app_anr_2024-01-01-00-00-00.txt",
        "==== end",
    ]
    records = parse_bugreport_lines(lines)
    assert records[0].level == "E"
    assert records[0].tag == "Tag"
    # dropbox block should still be preserved
    dropbox_lines = [rec for rec in records if rec.tag == "dropbox"]
    assert len(dropbox_lines) >= 2
