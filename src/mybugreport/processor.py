...
def extract_context_sections(dates: Iterable[str], input_file: str, output_file: str, num_context_lines: str) -> None:
    ...
    # Build arguments as separate tokens so grep receives each pattern correctly.
    dates_arg_list = []
    for date in dates:
        dates_arg_list.extend(["-e", date])
    debug_iterable("date_patterns", dates_arg_list)

    command = ['grep', '-A', str(num_context_lines), '-B', str(num_context_lines)] + dates_arg_list + [input_file]
